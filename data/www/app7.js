(function(){
const cmds=["help","health","free","df","date","ntp status","ntp sync","wifi status","wifi net","ifconfig","ip route","relay status","rule list","crontab -l","timer list","jobs","dmesg","sensor read","i2c scan"];
function addOps(){
$("#tabs")?.insertAdjacentHTML("beforeend",`<button class="tab" data-tab="ops">Quick Cmds</button>`);
$("#panels")?.insertAdjacentHTML("beforeend",`<section class="panel" id="ops"><section class="card"><h2>Command Shortcuts</h2><div class="miniPanel"><button data-cmd="relay status">Relays</button><button data-cmd="rule list">Rules</button><button data-cmd="crontab -l">Cron</button><button data-cmd="timer list">Timers</button><button data-cmd="input list">Inputs</button><button data-cmd="health">Health</button></div><pre class="formOut"></pre></section><section class="card"><h2>Templates</h2><div class="miniPanel"><button data-fill="relay add rele1 D1 active_low">relay add</button><button data-fill="schedule rele1 08:00 08:10">daily schedule</button><button data-fill="climate temp fan 38 40">temp rule</button><button data-fill="climate hum extractor 60 70">humidity rule</button><button data-fill="input add button D2 pullup">input add</button><button data-fill="timer every 60000 health">timer health</button></div></section></section>`);
