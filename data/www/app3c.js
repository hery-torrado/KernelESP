...
if(window.kespReady){window.kespBoot();kespApplyFocusMode();setTimeout(kespRetryRequestedTab,50)}
};
...
function kespFocusMode(){return new URLSearchParams(location.search).get("focus")=="1"&&!!kespRequestedTab()}
function kespApplyFocusMode(){document.body.classList.toggle("focusPanel",kespFocusMode())}
window.kespOpenRequestedTab=function(){let id=kespRequestedTab();return id?window.kespOpenTab(id):false};
function kespRetryRequestedTab(until=Date.now()+12000){kespApplyFocusMode();let id=kespRequestedTab();if(!id)return;if(window.kespOpenRequestedTab())return;if(Date.now()<until)setTimeout(()=>kespRetryRequestedTab(until),250)}
addEventListener("hashchange",()=>setTimeout(()=>{kespApplyFocusMode();kespRetryRequestedTab()},30));
window.kespStart();
