...
if(window.kespReady){window.kespBoot();setTimeout(kespOpenRequestedTab,50);setTimeout(kespOpenRequestedTab,1300);setTimeout(kespOpenRequestedTab,2600)}
};
...
function kespOpenRequestedTab(){let id=kespRequestedTab();if(id)window.kespOpenTab(id)}
addEventListener("hashchange",()=>setTimeout(kespOpenRequestedTab,30));
window.kespStart();
